# examples/zephyr-devshell-4.4.1

**A complete [Zephyr RTOS](https://zephyrproject.org) 4.4.1 development
environment — the largest example in the tree, and the one to actually run.**

```console
$ denver run examples/zephyr-devshell-4.4.1
...
| 4 | `zephyr` | `west update`: **clones the many repositories** of a Zephyr workspace at the revisions pinned for 4.4.1, applies this env's patches, fetches binary blobs |
...
zephyr-devshell/         the 5-stage pipeline, shared recipes, common env
      ▲ imported by
zephyr-devshell-4.4.1/   ONLY the 4.4.1-specific bits:
...
**1. The lockfile pattern (`freeze-to:` → `requirements:`).** `uv-zephyr`
writes its fully-resolved `uv pip freeze` output to
`requirements.final.txt`; the `uv` stage then *reads that same file back in*
...
directory layout — `skip-on-success:`, `patches-apply:`, and `uv-zephyr`'s
...
repeats `overrides:`, `skip-on-success:` and `patches-apply:` itself, because a stage's
section is its own.

**3. Claiming the base's recipes.** The shared env ships recipes but declares
no `recipes:` entry for them (it has no conanfile). This env's `recipes:`
entry lists both directories explicitly, base first, and writes the merged
catalog:
...
- **`patches-apply:`** is denver's: a literal `venv-patcher` command patches
  installed *Python packages*, here three patches against `west` itself.
- **`zephyr/patches.yml`** is not a `denver.toml` key at all. It is west's own
...
recipes, so `conan` (stage 2) can supply them to `uv` (stage 3) via
`UV_FIND_LINKS` — which is the real reason `conan` runs before `uv` here,
and lets the install work offline.

**6. `skip-on-success:`.** `uv/skip-on-success.sh` is three lines: if `west` and `conan` are
...
the host's tools instead of the pinned ones. A `skip-on-success:` script decides
...
| `uv/skip-on-success.sh` | The "already installed?" check |
...
- [`doc/providers/zephyr.md`](../../doc/providers/zephyr.md) and
  [`uv.md`](../../doc/providers/uv.md) — the two providers doing the most here
