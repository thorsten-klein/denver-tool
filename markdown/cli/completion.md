...
shebang makes `./src/denver.py` directly executable) — `python3 src/denver.py <TAB>` still can’t be completed this way, since bash keys
...
- `<env>` — directories and `denver.yml`/`denver.*.yml`/`denver.toml`/`denver.*.toml` files on disk
