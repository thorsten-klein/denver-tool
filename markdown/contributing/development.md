...
it freezes the standalone executable (`scripts/create-python-exe.sh --output dist`) – see “Prebuilt Binary” in [Install](../introduction/install.md)
...
3. Implement `setup(self, ctx)` (a setup provider) and/or `wrap(self, ctx, cmd)` (a wrapper provider) — this is where the real work (and any real
