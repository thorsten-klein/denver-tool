...
in a file editor of your choice, or ask `denver` to show the resolved config with `denver run examples/simple-env --show-config`.
...
- The first stage prints the variables and their current values (most likely empty).
- The second stage prints some text, then sources `custom.sh` — open that
  script and you'll see it sets those variables to specific values.
- The third stage prints the variables again, to check the second stage worked.
...
Note: for quieter output, run with `-q` or `-qq`. Try it out!

To land in an interactive shell, either name it explicitly, or omit the command entirely to fall back to the environment's default:
...
Let's clarify the naming for this specific example:
...
stage whose tool is missing fails with a clear message naming what it
looked for.

You can still let denver handle that setup, though: something that must be
installed on the *host* itself (e.g. `docker`) can be a script wired into
`denver.toml` as `scripts: setup:`, then run once with:
