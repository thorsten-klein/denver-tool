...
It is configured entirely from the env's denver config -- a provider never
...
class Provider:
    ...
    #: the denver config section this provider reads (defaults to its name)
    name: str | None = None
    ...
    def __init__(self, config):
        """Store the whole merged denver config ``config`` and default this provider's stage id to its type name."""
        # the full merged denver config; a provider reads its own section
    ...
    def config_section(self, ctx):
        """This provider's interpolated config section from the denver config."""
