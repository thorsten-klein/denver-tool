...
Nothing is wrapped. denver does not launch `nix develop --command <your command>`, and there is no second “post-nix” script: the environment is
...
so simply sourcing it runs the devShell’s `shellHook` exactly as `nix develop` would, and denver does nothing extra to make that happen. That is
