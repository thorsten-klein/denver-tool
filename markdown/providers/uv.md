...
- **`amend`** (default `true`) — see “Reproducibility” in
  [`../concepts/philosophy.md`](../concepts/philosophy.md) for the full trade-off. When `true`, every `uv pip install` invocation reuses every `-r`/`--override`/`--no-index`/literal
