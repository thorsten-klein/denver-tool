...
* [ADR-0001: Central default resolution](adr-0001-central-default-resolution.md)
* [ADR-0002: No path is inferred from the directory layout](adr-0002-no-inferred-paths.md)
* [ADR-0003: YAML is the default config format, TOML optional](adr-0003-config-format.md)
* [ADR-0004: A container is a wrapper stage that relocates the run](adr-0004-wrapper-relocation.md)
...
| [ADR-0005]()                                       | `provider:` is mandatory, never guessed from the stage id                 | accepted |
| [ADR-0006]()                                       | `stages:` names the pipeline, `provider:` names the type                  | accepted |
| [ADR-0007]()                                       | Lists append across layers instead of replacing                           | accepted |
| [ADR-0008]()                                       | `die()` raises `DenverError`, caught once in `main()`                     | accepted |
| [ADR-0009]()                                       | An env’s state lives with the env, not in a shared pool                   | accepted |
| [ADR-0010]()                                       | Concurrent runs of one env are serialised by a lock                       | accepted |
| [ADR-0011]()                                       | Fingerprints cover content and layout, not absolute paths                 | accepted |
| [ADR-0012]()                                       | A project registers its own provider instead of forking denver            | accepted |
| [ADR-0013]()                                       | Recurring shell scripts became the `download` and `git` providers         | accepted |
| [ADR-0014]()                                       | The CLI is subcommand-based (`run`, `clean`, `complete`)                  | accepted |
| [ADR-0015]()                                       | Shipped packages are `denver_providers`, `denver_assets`, `denver_errors` | accepted |
| [ADR-0016]()                                       | The `pip` provider is called `uv`                                         | accepted |
...
**Context.** The provider was called `pip` but drove `uv` throughout — `uv venv`, `uv pip install`, `uv python install`. The name described the
