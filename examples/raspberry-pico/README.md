...
```toml
base-classes = ["../zephyr-devshell/conan/base_classes"]  # shared conanfile base classes
...
A `recipes:` entry's `dirs:` are resolved into **one** catalog, so
`catalog.yml` here pins all three recipes — the two local ones and the
borrowed CMake — as `name/version@user/channel#revision`. `conanfile.py` then reads its
...
| Path | What it is |
|---|---|
| `denver.toml` | Two stages: `uv`, then `conan` — plus a per-env `CONAN_HOME` |
...
- [`doc/providers/conan.md`](../../doc/providers/conan.md) — `conanfile:`,
  `recipes:`, catalogs, remotes, profiles
