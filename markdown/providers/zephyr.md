...
- **`base`** (default: `${WEST_TOPDIR}/zephyr-rtos`) — `ZEPHYR_BASE`. The
  `zephyr.base` `west config` entry itself is set separately, *after* `west update` and project patches are applied, from `west list zephyr -f {path}` — i.e. wherever the manifest’s own `zephyr` project actually
